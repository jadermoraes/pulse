<script lang="ts">
  ...
  const POLL_MS = 300_000; // 5 minutes (stats cadence)
  const ROW_PX = 48; // fallback thumbnail-row height (matches .mw-row CSS; fitRows measures the real row)
...
<div class="card">
  ...
  {:else}
    <div class="mw-list" use:fitRows={{ rowPx: ROW_PX, onFit: (n) => (fit = n), onGeometry }}>
      {#each items as it, i}
        <div class="mw-row">
          ...
          <div class="mw-detail">
            <div class="mw-head">
              <span class="mw-title">{it.title}</span>
...
<style>
  ...
  .mw-row {
    ...
    /* Fixed poster-row height so fit-to-height math is exact — must match ROW_PX.
       flex-shrink:0 keeps each row exactly this tall; overflow:hidden clips any taller inner content. */
    height: 48px;
    flex: 0 0 48px;
    padding: 4px 2px;
